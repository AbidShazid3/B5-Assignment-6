...
const AllTransaction = () => {
    ...
    const { data: transactionData, isLoading, isError } = useGetAllTransactionQuery(query);
    const totalPage = transactionData?.meta?.totalPage || 1;
        ...
        <div className="w-full">
            <div className="flex flex-col gap-4 w-full">
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                    <h1 className="text-2xl lg:text-3xl font-medium text-center md:text-left">
                        Total Wallet: {transactionData?.meta?.total}
                    </h1>
                </div>
            ...
            <div className="overflow-hidden rounded-md border mt-5">
                <Table>
                    ...
                    {!isLoading && !transactionData?.data && (
                        <div>
                            <p>No Data Found</p>{" "}
                        </div>
                    )}
                    ...
                    {!isLoading && !isError && transactionData?.data?.length > 0 && (
                        <TableBody>
                            {transactionData?.data?.map((tnx: ITransaction, idx: number) => (
                                <TableRow key={tnx._id}>
                                    <TableCell className="font-medium">{idx + 1}</TableCell>
                                    <TableHead>
                                        <div className="flex flex-col gap-2 py-2">
                                            <p>{tnx?.from?.user?.name} - {tnx?.from?.user?.role}</p>
                                            <p>{tnx?.from?.user?.phone}</p>
                                        </div>
                                    </TableHead>
                                    <TableHead>
                                        <div className="flex flex-col gap-2 py-2">
                                            <p>{tnx?.to?.user?.name} - {tnx?.to?.user?.role}</p>
